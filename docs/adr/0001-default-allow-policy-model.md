...
- `allow_when:` is reserved as the positive form of the same slot, for cases where "permit only when X" reads more naturally than "deny when not X" (typically high blast-radius Tools — see [[strategic-decisions]] §14). When implemented (slice 3+), it desugars at load time to the equivalent `deny_when not X`. Exactly one of `deny_when` or `allow_when` is allowed per Rule. The engine remains deny-only forever; there is no precedence between allow and deny to resolve.
